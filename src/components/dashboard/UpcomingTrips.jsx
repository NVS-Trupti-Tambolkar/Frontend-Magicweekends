...
const UpcomingTrips = ({ onLoad }) => {
    ...
    const handleBrochureDownload = (e, trip) => {
        ...
        if (!trip.brochure_url) {
            setSnackbar({ open: true, message: 'Brochure not available', type: 'warning' });
            return;
        }
                                    ...
                                    <div className={`absolute inset-0 transition-all duration-500 transform bg-black/40 backdrop-blur-[2px] ${hoveredCard === `${trip.type}-${trip.id}`
                                        ...
                                        <div className="absolute inset-0 flex flex-col justify-center items-center p-6 text-center">
                                            ...
                                            <div className="flex flex-col sm:flex-row gap-2 w-full justify-center">
                                                <button
                                                    className="bg-yellow-500 text-gray-900 font-bold py-2.5 px-4 rounded-lg hover:bg-yellow-400 transition-all duration-300 text-xs shadow-lg flex items-center justify-center gap-2 flex-1"
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                ...
                                                {trip.brochure_url && (
                                                    <button
                                                        className="bg-white/20 backdrop-blur-md border border-white/30 text-white font-bold py-2.5 px-4 rounded-lg hover:bg-white/30 transition-all duration-300 text-xs shadow-lg flex items-center justify-center gap-2 flex-1"
                                                        onClick={(e) => handleBrochureDownload(e, trip)}
                                                    >
                                                        BROCHURE
                                                    </button>
                                                )}
