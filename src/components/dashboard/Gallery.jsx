...
import { OverlayLoader, PageLoader } from '../common/LoadingSpinner';
import NotificationSnackbar from '../common/NotificationSnackbar';

const Gallery = ({ onLoad }) => {
    const { user } = useAuth();
    ...
    const handleSingleUpdateSubmit = async (e) => {
        ...
        formData.append('folder_name', editingImage.tourName?.trim());
    ...
    const handleSubmit = async (e) => {
        ...
        if (isEditing) {
            formData.append('old_folder_name', editingFolder.tourName?.trim());
            formData.append('new_folder_name', newGallery.tourName?.trim());
        } else {
            formData.append('folder_name', newGallery.tourName?.trim());
        }
        ...
        try {
            ...
            if (response.success) {
                showMessage(isEditing ? 'Gallery updated successfully!' : (isAppending ? 'Extra photos added successfully!' : 'Gallery photos added successfully!'));
                fetchData();
            ...
            showMessage('Failed to save gallery', 'error');
        } finally {
            setUpdating(false);
        }
    };
                            ...
                            >
                                {(user?.role === 'admin' || user?.role === 'manager') && (
                                    <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                                        <button
                                            onClick={(e) => { e.stopPropagation(); handleAppend(gallery); }}
                                            className="p-2 bg-white/90 rounded-full text-green-600 hover:bg-green-600 hover:text-white transition-all shadow-sm"
                                ...
                                <div className="p-3 sm:p-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        {gallery.images.slice(0, 4).map((image, index) => (
                                            ...
                                            >
                                                <img
                                                    src={galleryImages[image] || 'https://via.placeholder.com/400x300?text='}
                ...
                <div className="fixed inset-0 bg-black/90 z-[60] flex items-center justify-center p-4 backdrop-blur-md animate-fadeIn">
                    ...
                    <div className="relative w-full h-full flex items-center justify-center pointer-events-none">
                        ...
                        >
                            ...
                            {!user && (
                                <div className="absolute top-6 right-6 flex flex-col gap-3 z-[80] opacity-0 group-hover:opacity-100 transition-all duration-500">
                                    <button onClick={handleZoomIn} className="p-3 bg-white/5 text-white/70 hover:text-white rounded-2xl backdrop-blur-xl transition-all border border-white/10">
                                        <FaSearchPlus className="w-5 h-5" />
                                    </button>
                                    <button onClick={handleZoomOut} className="p-3 bg-white/5 text-white/70 hover:text-white rounded-2xl backdrop-blur-xl transition-all border border-white/10">
                                        <FaSearchMinus className="w-5 h-5" />
                                    </button>
                                </div>
                            )}
                            ...
                            <div className="bg-white/95 backdrop-blur-lg border border-yellow-200 shadow-2xl rounded-2xl overflow-hidden">
                                    ...
                                    <div className="relative flex justify-between items-center h-full px-4 sm:px-8">
                                        <div>
                                            <h2 className="text-xl sm:text-2xl font-bold text-black drop-shadow-sm">
                                                {isEditing ? 'Edit Gallery Folder' : (isAppending ? 'Add More Photos' : 'Create New Gallery')}
                                            </h2>
                                            <p className="text-black/70 text-xs sm:text-sm mt-0.5">
                                                {isAppending ? `Adding to: ${editingFolder?.tourName}` : 'Share your amazing travel moments'}
                                            </p>
                                        </div>
                                        <button
                                            onClick={() => { setShowAddForm(false); setIsEditing(false); setIsAppending(false); }}
                                ...
                                <form onSubmit={handleSubmit} className="p-6 sm:p-8">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <div className="mb-2">
                                            ...
                                            <div className="relative">
                                                <select
                                                    className={`w-full py-3 px-4 bg-gray-50 text-gray-900 border border-gray-200 rounded-xl focus:outline-none focus:border-yellow-500 focus:ring-4 focus:ring-yellow-500/10 transition-all appearance-none cursor-pointer ${isAppending || isEditing ? 'opacity-60 cursor-not-allowed' : ''}`}
                                                    name="trip_id"
                                                    value={newGallery.trip_id}
                                                    onChange={handleInputChange}
                                                    disabled={isEditing || isAppending}
                                                >
                                        ...
                                        <div className="mb-2">
                                            ...
                                            <input
                                                className={`w-full py-3 px-4 bg-gray-50 text-gray-900 border border-gray-200 rounded-xl focus:outline-none focus:border-yellow-500 focus:ring-4 focus:ring-yellow-500/10 transition-all placeholder-gray-400 ${isAppending ? 'opacity-60 cursor-not-allowed' : ''}`}
                                                name="tourName"
                                                type="text"
                                    ...
                                    <div className="flex justify-end mt-10 gap-4 pt-8 border-t border-gray-100">
                                        ...
                                        >
                                            {isEditing ? 'Save Changes' : (isAppending ? 'Add Photos' : 'Upload Gallery')}
                                        </button>
                                    </div>
                                </form>
                            </div>
