...
const ExploreWithUs = ({ onLoad }) => {
    ...
    const fetchTrips = async () => {
        try {
            ...
            if (response.data.success) {
                ...
                const filteredByDate = response.data.data.filter(trip => {
                    if (!trip.trip_date) return true;
                    const tripDate = new Date(trip.trip_date);
                    return tripDate >= today;
                });
                const tripData = filteredByDate;
    ...
    const handleBrochureUpload = (e) => {
        ...
        if (file) {
            if (file.type !== 'application/pdf') {
                showMessage("Please upload a PDF file for the brochure.", "warning");
                return;
            }
            setNewDestination(prev => ({ ...prev, brochure: file }));
            showMessage("Brochure selected: " + file.name);
        }
    };
    ...
    const handleEdit = async (e, trip) => {
        ...
        // 2. Fetch full trip details (including multiple dates)
        try {
            const detailsResp = await api.get(`/Trip/getTripById?id=${trip.id}`);
            if (detailsResp.data.success) {
                ...
                setNewDestination({
                    ...
                    trip_date: fullTrip.trip_date ? fullTrip.trip_date.split('T')[0] : '',
                    departure_dates: (fullTrip.departure_dates || []).map(d => d.split('T')[0]),
                    itineraries: fetchedItineraries,
                    inclusions: fullTrip.inclusions ? (Array.isArray(fullTrip.inclusions) ? fullTrip.inclusions.join(', ') : fullTrip.inclusions) : '',
                    exclusions: fullTrip.exclusions ? (Array.isArray(fullTrip.exclusions) ? fullTrip.exclusions.join(', ') : fullTrip.exclusions) : '',
                    brochure: null
                });
            }
        } catch (err) {
            console.error("Error fetching full trip details for edit:", err);
            ...
            setNewDestination({
                ...
                departure_dates: [],
                itineraries: fetchedItineraries,
                inclusions: trip.inclusions ? (Array.isArray(trip.inclusions) ? trip.inclusions.join(', ') : trip.inclusions) : '',
                exclusions: trip.exclusions ? (Array.isArray(trip.exclusions) ? trip.exclusions.join(', ') : trip.exclusions) : ''
            });
        }
    ...
    const handleAddDestination = async (e) => {
        ...
        if (!isEditing && !newDestination.image) {
            showMessage("Please upload a destination image.", "warning");
            return;
        }
        ...
        try {
            ...
            formData.append('trip_date', newDestination.trip_date);
            formData.append('departure_dates', newDestination.departure_dates.join(','));
            formData.append('inclusions', JSON.stringify(newDestination.inclusions.split(',').map(s => s.trim()).filter(s => s !== "")));
            formData.append('exclusions', JSON.stringify(newDestination.exclusions.split(',').map(s => s.trim()).filter(s => s !== "")));
            ...
            if (newDestination.brochure) {
                formData.append('brochure', newDestination.brochure);
            }
            ...
            if (response.data.success) {
                ...
                showMessage(isEditing ? 'Trip updated successfully!' : 'Trip added successfully!');
                ...
                if (validItinsToSave.length > 0) {
                    try {
                        ...
                        showMessage('Trip saved but some itineraries failed. Please edit the trip to verify itineraries.', 'warning');
                    }
                }
            ...
            showMessage(isEditing ? 'Failed to update trip.' : 'Failed to add trip. Please try again.', 'error');
        } finally {
            setUpdating(false);
        }
    };
                                                    ...
                                                    <div className="absolute bottom-0 left-0 right-0 p-6">
                                                        ...
                                                        <div className="flex flex-row items-center gap-3 mt-4">
                                                            <button
                                                                ...
                                                                className="bg-transparent border border-white/40 text-white font-montserrat py-2.5 px-4 hover:bg-white/30 hover:border-white/40 transition-all duration-300 text-[10px] sm:text-xs tracking-[0.2em] whitespace-nowrap uppercase">
                                                                EXPLORE MORE
                                                            </button>
                                                        {destination.brochure_url && (
                                                            <button
                                                                onClick={(e) => {
                                                                    e.stopPropagation();
                                                                    const apiUrl = process.env.REACT_APP_API_URL || 'http://localhost:4000';
                                                                    const downloadUrl = `${apiUrl}/Trip/download-brochure/${destination.id}`;
                                                                    window.open(downloadUrl, '_self');
                                                                }}
                                                                className="bg-yellow-500 border-yellow-500 text-black hover:bg-yellow-600 font-montserrat py-2.5 px-4 transition-all duration-300 text-[10px] sm:text-xs tracking-[0.1em] font-bold flex items-center gap-1.5 whitespace-nowrap uppercase">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
                                        ...
                                        <div className="md:col-span-2 mb-6">
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                                                {newDestination.departure_dates.map((date, index) => (
                                                ...
                                                {newDestination.departure_dates.length === 0 && (
                                                    <p className="text-gray-500 text-sm italic col-span-full">No departure dates added. Click "Add Departure Date" above.</p>
                                                )}
                                            </div>
                                        </div>

                                        {/* Trip Date (Legacy Single Date) */}
                                        <div className="mb-2">
                                            <label className="block text-gray-700 text-sm font-bold mb-2">Primary Trip Date (Reference)</label>
                                            <input
                                                type="date"
                                                name="trip_date"
                                                value={newDestination.trip_date}
                                                onChange={handleDestinationInputChange}
                                                className="w-full py-3 px-4 bg-gray-50 text-gray-900 border border-gray-200 rounded-xl focus:outline-none focus:border-yellow-500 focus:ring-4 focus:ring-yellow-500/10 transition-all"
                                            />
                                        </div>
                                        ...
                                        {/* Brochure Upload */}
                                        <div className="mb-4 md:col-span-2">
                                            <label className="block text-gray-700 text-sm font-bold mb-2">Trip Brochure (PDF)</label>
                                            <div className="flex flex-col items-center">
                                                <div className="w-full">
                                                    <label htmlFor="brochure-upload" className="flex flex-col items-center justify-center w-full h-32 bg-gray-50 border-2 border-dashed border-gray-200 rounded-xl cursor-pointer hover:bg-yellow-50 hover:border-yellow-300 transition-all group">
                                                        <svg className="w-8 h-8 mb-2 text-gray-400 group-hover:text-yellow-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
                                                        </svg>
                                                        <p className="text-gray-500 text-sm font-medium">Click to upload Brochure PDF</p>
                                                        <p className="text-gray-400 text-xs text-center px-4">PDF up to 10MB {isEditing && "(Leave empty to keep existing brochure)"}</p>
                                                    </label>
                                                    <input
                                                        id="brochure-upload"
                                                        name="brochure"
                                                ...
                                                {newDestination.brochure && (
                                                    <div className="mt-2 text-sm text-green-600 font-medium bg-green-50 px-4 py-2 rounded-lg border border-green-100 flex items-center gap-2">
                                                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                                            <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                                                        </svg>
                                                        {newDestination.brochure.name}
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        {/* Inclusions */}
                                        <div className="mb-4 md:col-span-2">
                                            <label className="block text-gray-700 text-sm font-bold mb-2">Inclusions (comma separated)</label>
                                            <textarea
                                                className="w-full py-3 px-4 bg-gray-50 text-gray-900 border border-gray-200 rounded-xl focus:outline-none focus:border-yellow-500 focus:ring-4 focus:ring-yellow-500/10 transition-all placeholder-gray-400"
                                                name="inclusions"
                                                value={newDestination.inclusions}
                                                onChange={handleDestinationInputChange}
                                        ...
                                        {/* Exclusions */}
                                        <div className="mb-4 md:col-span-2">
                                            <label className="block text-gray-700 text-sm font-bold mb-2">Exclusions (comma separated)</label>
                                            <textarea
                                                className="w-full py-3 px-4 bg-gray-50 text-gray-900 border border-gray-200 rounded-xl focus:outline-none focus:border-yellow-500 focus:ring-4 focus:ring-yellow-500/10 transition-all placeholder-gray-400"
                                                name="exclusions"
                                                value={newDestination.exclusions}
                                                onChange={handleDestinationInputChange}
                                                placeholder="e.g. Flights, Personal expenses, Insurance"
                                                rows="3"
                                            />
                                        </div>
                                    </div>
